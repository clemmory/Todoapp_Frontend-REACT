...
// Display todo when being edited
  ...
  if(isEdited) {
    return (
      <div className ='w-64 mb-4 bg-white border rounded-lg p-5 flex flex-col justify-between'>
    ...
    return (
      <div className ='w-60 h-24 mb-4 bg-white border rounded-lg p-3 flex flex-col justify-between'>
        <div className = 'flex justify-between items-center'>
          <p className="font-bold text-slate-500">{props.due_date}</p>
          <div className='flex'>
            <svg  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" 
                  className="w-4 h-4 stroke-slate-400 mr-2 hover:stroke-sky-600 cursor-pointer"
                  onClick={() => handleEdit()}>
        ...
        <svg  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor "
                className={`w-5 h-5 mr-2  hover:fill-green-500 cursor-pointer ${isDone ? 'fill-green-600' : 'fill-slate-300'}`}
                onClick={() => handleCheck()}>
